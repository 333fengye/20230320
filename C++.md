- 基础
    ...
    - new
        ...
        - 函数重载
            - c++中允许函数的参水列表指定默认值，而且这个默认值必须从右向左依次指定，不能间断，一般在函数声明中指定，在函数的定义中指定编译器可能不会抱错，但大多数情况下这是毫无意义的操作
        ...
        - nullptr和NULL
            ...
            - nullptr是c++11新标准引入的的一个关键词，代表了空指针，如果出现了多个类型的空指针，然然会语以不明确，则需要强转来明确空指针类型
            - 区别
...
- 类
    ...
    - 类之间的关系
        ...
        - 纵向关系：继承
            ...
            - 继承方式
                - public
                    - 父类            子类 public          public protected       protected private         不可访问
                - protected
                    - 父类            子类 public          protected protected       protected private         不可访问
                - private
            ...
            - 类成员函数指针
                ...
                - 通过函数指针调用的好处
                    ...
                    - void (CTest::*p_fun)() = &CTest::show;//定义类成员函数指针并初始化，注意：& 和 类名作用域都不能省略 typedef void (CTest::*P_FUN)(); //使用typedef进行优化 P_FUN p_fun2 = &CTest::show; CTest  tst; CTest* past = new CTest; (tst.*p_fun2)();//普通对象通过指针调用类成员函数 (past->*p_fun2)();//只针对象通过指针调用类成员函数
                    - 可以通过类成员函数指针来模拟实现多态
                        - class CPeople {}; class CYellow : public CPeople { public:           void Eat() {                  cout<<" asdf"<<endl;           } }; int main() {     CPeople* pPeo = new CYellopw;     //pPeo->Eat();  //不能直接调用     void (CPeople::* p_fun)() = (void (CPeople::*)())&CYellow::Eat;//通过强转指向子类的函数     （pPeo->*p_fun)(); }
    - 多态
...
- qt
    ...
    - 信号与槽
        - 基本概念
            - 信号(signal)和槽(slot)是 Qt 框架引以》豪的机制之一，用于完成界面操作的响立，是完成任意两个QT对象之间的通信机制。
            - 信号
